...
static int scanstat(const char* str, struct stat *stbuf) {
    ...
    int ret;
    #ifdef __CYGWIN__
      /* Avoid warning: format '%li' expects argument of type 'long int *', but argument 19 has type 'blksize_t *' {aka 'int *'} [-Wformat=] */
      ret = sscanf(str, "ino=%lli mode=%c%c%c%c%c%c%c%c%c%c nlink=%lli uid=%i gid=%i "
          "rdev=%lli size=%lli blksize=%i blocks=%lli atime=%lf mtime=%lf ctime=%lf %n"
           ,&st_ino
...
static int execfuse_release(const char *path, struct fuse_file_info *fi)
{
    ...
    /* notify the script when a file is closed */
    call_script_simple("closed", path);
